...
import {
    ErrorTypesChart,
    ErrorTypesChartSkeleton,
} from "@/components/chart/error-types-chart";
import {
    SeverityLevelsChart,
    SeverityLevelsChartSkeleton,
} from "@/components/chart/severity-levels-chart";
import {
    ErrorTrendsChart,
    ErrorTrendsChartSkeleton,
} from "@/components/chart/error-trends-chart";
import {
    RecentErrorsChart,
    RecentErrorsChartSkeleton,
} from "@/components/chart/recent-errors-chart";
import {
    getAnalyticsStats,
    getErrorTrends,
    getTopErrors,
...
export async function ErrorsAnalytics() {
    ...
    const topErrors =
        topErrorsResult.success && topErrorsResult.data ? topErrorsResult.data : [];
    const errorsByEnv =
        errorsByEnvResult.success && errorsByEnvResult.data
            ? errorsByEnvResult.data
            : [];
    ...
    const totalErrors = stats
        ? typeof stats.totalErrors === "string"
            ? Number.parseInt(stats.totalErrors)
            : stats.totalErrors
        : 0;
    const criticalErrors =
        stats?.errorsBySeverity.find((s) => s.severity === "critical")?.count || 0;
    const trends =
        errorTrendsResult.success && errorTrendsResult.data
            ? errorTrendsResult.data
            : [];
    const yesterdayErrors =
        trends.length > 1
            ? Number(trends[trends.length - 2]?.total_errors || 0)
            : 0;
    const todayErrors =
        trends.length > 0
            ? Number(trends[trends.length - 1]?.total_errors || 0)
            : 0;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">
                            {formatNumber(
                                typeof metrics.totalErrors === "string"
                                    ? Number.parseInt(metrics.totalErrors)
                                    : metrics.totalErrors,
                            )}
                        </div>
                        <div className="flex items-center text-xs text-muted-foreground mt-1">
                            ...
                            <span
                                className={errorTrend > 0 ? "text-red-500" : "text-green-500"}
                            >
                ...
                <Card>
                    ...
                    <CardContent>
                        ...
                        <div className="text-xs text-muted-foreground mt-1">vs logs</div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        ...
                        <div className="text-xs text-muted-foreground mt-1">
                            {totalErrors > 0
                                ? ((criticalErrors / totalErrors) * 100).toFixed(1)
                                : 0}
                            % of total
                        </div>
                    </CardContent>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold text-green-500">
                            {formatPercentage(metrics.systemHealth || 100)}
                        </div>
                        <div className="text-xs text-muted-foreground mt-1">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Error occurrence trends over the past two weeks
                    </CardDescription>
                </CardHeader>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Top error patterns in your application
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {topErrors.length === 0 ? (
                                ...
                                {topErrors.slice(0, 6).map((error, index) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-2 min-w-0 flex-1">
                                            <Badge
                                                variant="outline"
                                                className={`flex items-center gap-1 px-2 py-1 ${error.severity === "critical"
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Error distribution across different environments
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="space-y-3">

...
function SubmitButton() {
	...
	return (
		...
		>
			{isPending && <SpinnerGapIcon className="mr-2 h-4 w-4 animate-spin" />}
			Save Changes
		</Button>
	);
}
	...
	return (
		...
		>
			{isPending ? (
				...
				<SpinnerGapIcon className="mr-2 h-4 w-4" />
			)}
			Regenerate Token
		</Button>
	);
...
export function TokenForm({
			...
			<form action={formAction}>
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Use this token to authenticate with the Better Analytics API.
  ...
  -H "Authorization: Bearer ${
		user?.accessToken ? user.accessToken.substring(0, 12) + "..." : "YOUR_TOKEN"
	}" \\

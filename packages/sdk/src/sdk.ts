...
export class BetterAnalyticsSDK {
	...
	constructor(config: SDKConfig) {
		...
		if (!this.config.clientId) {
			...
			console.warn(
				"[BetterAnalyticsSDK] Warning: `clientId` is not provided. The SDK will be disabled.",
			);
			return;
		}
	...
	private logDebug(message: string, data?: unknown): void {
		if (this.config.debug) {
			const prefix = this.isServer
				? `[BetterAnalytics:${this.isServer ? "Server" : "Client"}]`
				: "[BetterAnalytics]";
	...
	private setupAutoLogging(): void {
		...
		this.logDebug(
			`Console auto-logging enabled for levels: ${logLevels.slice(configuredLogLevelIndex).join(", ")}`,
		);
	}
	...
	private enrichErrorData(
		data: Partial<ErrorData>,
		serverContext?: ServerErrorContext,
	): ErrorData {
	...
	private enrichServerErrorData(
		...
		const enrichedData: Partial<ErrorData> = {
			...
			process_id:
				data.process_id ||
				(typeof process !== "undefined" ? process.pid : undefined),
		...
		if (serverContext) {
			...
			if (serverContext.req?.userAgent) {
				const uaResult = this.parser
					.setUA(serverContext.req.userAgent)
					.getResult();
	...
	private async send(
		...
		try {
			...
			this.logDebug(
				`${endpoint === "ingest" ? "Error" : "Log"} sent successfully`,
				{ id: result.id },
			);
			return result;
		} catch (error) {
			this.logDebug(
				`Failed to send ${endpoint === "ingest" ? "error" : "log"}`,
				{ error, retryCount },
			);
	 ...
	 */
	async captureError(
		data: Partial<Omit<ErrorData, "custom_data">> & {
	...
	async captureException(
		error: Error,
		context?: Partial<Omit<ErrorData, "custom_data">> & {
			custom_data?: Record<string, any> | string;
		},
		serverContext?: ServerErrorContext,
	): Promise<SDKResponse> {
		if (this.isDisabled) return { success: false, message: "SDK is disabled" };
		const errorData: Partial<Omit<ErrorData, "custom_data">> & {
			custom_data?: Record<string, any> | string;
		} = {
			error_type: this.isServer ? "server" : "client",
			severity: "high",
	...
	async log(
		message: string,
		context?: Partial<Omit<LogData, "message" | "context">> & {

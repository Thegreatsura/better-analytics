...
export function createLocalizer(config: LocalizationConfig) {
	...
	return {
		translate: (key: string, language?: string) =>
			localizer.translate(key, { targetLocale: language }),
		translateMultiple: (keys: string[], language?: string) =>
			localizer.translateMultiple(keys, { targetLocale: language }),
		localizeObject: <T extends Record<string, string>>(
			obj: T,
			language?: string,
		) => localizer.localizeObject(obj, { targetLocale: language }),
